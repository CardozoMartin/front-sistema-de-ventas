 ...
 * 
 * @example
 * const { data: user, isLoading } = useUser('507f1f77bcf86cd799439011');
 */
export const useUser = (id: string, enabled = true) => {
  return useQuery({
    queryKey: userKeys.detail(id),
    queryFn: () => usersService.getUserById(id),
    enabled: enabled && id.length > 0, // Solo ejecutar si está habilitado y el ID no está vacío
    staleTime: 5 * 60 * 1000,
  });
};
 ...
 * updateUser.mutate({ id: '507f1f77bcf86cd799439011', data: updatedData });
 */
