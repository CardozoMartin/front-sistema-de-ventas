...
import {
  getPromotionsFromLocalStorage,
  PROMOTIONS_LOCAL_UPDATED_EVENT,
} from '../components/helpers/PromotionsStorage';
 ...
 */
export const useAllPromotionsNoPagination = (options?: { enabled?: boolean }) => {
  return useQuery({
...
/** Promociones para POS: caché de React Query + localStorage sincronizado */
export const usePromotionsFromLocalStorage = (): Promotion[] => {
  const { data: fromQuery } = useAllPromotionsNoPagination();
  const [fromStorage, setFromStorage] = useState<Promotion[]>(getPromotionsFromLocalStorage);

  useEffect(() => {
    const refresh = () => setFromStorage(getPromotionsFromLocalStorage());
    window.addEventListener("storage", refresh);
    window.addEventListener(PROMOTIONS_LOCAL_UPDATED_EVENT, refresh);
    return () => {
      window.removeEventListener("storage", refresh);
      window.removeEventListener(PROMOTIONS_LOCAL_UPDATED_EVENT, refresh);
    };
  }, []);
