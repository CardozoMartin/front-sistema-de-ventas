 ...
 * 
 * @example
 * const { data: sales, isLoading, error } = useSales(1, 10);
 */
export const useSales = (page?: number, limit?: number) => {
  return useQuery({
    queryKey: [...saleKeys.list(), { page, limit }],
    queryFn: () => salesService.getAllSales(page, limit),
...
export const useCreateSale = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      // Invalidar el caché de ventas, productos (por el cambio de stock) y clientes (por el cambio de deuda)
